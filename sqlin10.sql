...
WHERE Vendors.vend_id = Products.vend_id;     --объеденяем две таблицы при помощи сравнения двух индификаторов
...
UPDATE Costum                         --обновление в талбице
SET cust_email = 'free@mail.ru',       --указание столбца или столбцов и его новое значение
    cust_state = 'FU',
    num_cust = NULL                    --NULL удалить значение и оставит пустую строку.
WHERE cust_id = '2332';               --сообщает какая строка подлежит изменению, при не указании строки обновит весь столбец

DELETE FROM Costum        --удалит всю строку чей id равен 434
WHERE cust_id = '434';

--Создание таблиц и работа с ними =17
CREATE TABLE Newtab   
(
  tab_id      CHAR(10)      NOT NULL,                --создание новой таблицы с указанием столбца, типа и значения.
  tab_name    CHAR(254)     NOT NULL,         --если не применять NOT NULL, то автоматически присвоится NULL
  tab_Price   DECIMAL(8,2)  NOT NULL DEFAULT 1, --если значения не бдует указано то автоматичкски значение будет 1.
  tab_desc    VARCHAR(1000) NULL              --NULL может иметь отсутсвующее значение при добавлении или изменении данной строки.
);

ALTER TABLE Newtab
ADD tab_phone CHAR(20);      --добавление нового столбца в таблицу.
DROP COLUMN tab_phone;    --удаление столбца
...
--Обработка транзакций =20  INSERT UPDATE DELETE
BEGIN TRANSACTION                           --начало транзакции (пример для SQL Server)
INSERT INTO Costum (cust_id, cust_name)
VALUES ('1002', 'Klay');
SAVE TRANSACTION Start1;                    --точка сохранения
INSERT INTO Orders (order_id, order_date)
VALUES (3434, '21.02.20');
IF @@ERROR = 0 ROLLBACK TRANSACTION Start1; --если данная инструкция обрывается и возвращает 0, то возвращается на точку сохранения
COMMIT TRANSACTION                          --сообщение о том что транзакция прошла успешно

--Курсоры =21
DECLARE CustCursor CURSOR     --создание курсора (для PostgreSQL DECLARE CURSOR CustCursor IS)
FOR
SELECT * FROM Costum
WHERE cust_email IS NULL;

OPEN CURSOR CustCursor;   --запуск инструкци по курсору и сохранения для дальнейшего просмотра
CLOSE CustCursor;         --закрытие курсора для экономии затрачиваемых ресурсов